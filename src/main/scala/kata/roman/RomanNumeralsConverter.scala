...
object RomanNumeralsConverterImpl {
  ...
  sealed trait ToRomanNumeralsStrategy {
    def toRomanNumerals(number: Int): String
  }
   ...
   */
  case object Recursive extends ToRomanNumeralsStrategy {
    override def toRomanNumerals(number: Int): String = {
      @tailrec
   ...
   */
  case object FoldLeft extends ToRomanNumeralsStrategy {
    override def toRomanNumerals(number: Int): String = {
   ...
   */
  private class RomanNumeralsConverterImpl(strategy: ToRomanNumeralsStrategy) extends RomanNumeralsConverter {

    override def toRomanNumerals(number: Int) = {
      require(number > 0 && number < 3001, s"number: $number is not in range 1-3000")
      strategy.toRomanNumerals(number)
    }
  ...
  def apply(strategy: ToRomanNumeralsStrategy): RomanNumeralsConverter =
    new RomanNumeralsConverterImpl(strategy)
